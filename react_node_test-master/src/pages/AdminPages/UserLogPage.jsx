...
const UserLogPage = () => {
  ...
  const handleDelete = (logId) => {
    ...
    const updatedLogs = logs.filter(log => log.token !== logId);
    
    // Update state
    setLogs(updatedLogs);
    setFilteredLogs(filteredLogs.filter(log => log.token !== logId));
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredLogs.length === 0 ? (
                  ...
                  filteredLogs.map((log) => (
                    <tr key={log.token} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        {deleteConfirm === log.token ? (
                          <div className="flex justify-end space-x-2">
                            <button
                              onClick={() => handleDelete(log.token)}
